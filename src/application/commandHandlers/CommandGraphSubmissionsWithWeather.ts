...
router.get('/course/:courseId/maxPoints', async function (req, res) {
    res.setHeader('Content-Type', 'application/json');
    try {
        const startDate = new Date(<string>req.query.startDate);
        const endDate = new Date(<string>req.query.endDate);

        res.json(await graphSubmissionsWithWeatherHandler.GetGraphSubbmisonsAndWeatherDataMaxPoints(Number(req.params.courseId), startDate, endDate));
    } catch (err) {
        res.status(500);
        res.json(err);
    }
